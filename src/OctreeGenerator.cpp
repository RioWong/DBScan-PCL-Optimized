...
void OctreeGenerator::initCloudFromFile(string fileName) {
  ...
  ifstream file(
      fileName); // declare file stream: http://www.cplusplus.com/reference/iostream/ifstream/
  string value;
  double points[3];
  while (file.good()) {
    for (int i = 0; i < 3; ++i) {
    ...
    // cout << string( value, 1, value.length()-2 ); // display value removing the first and the
