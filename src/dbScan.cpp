...
dbscan::dbscan(const char *filename, const int octreeResolution_, const float eps_,
...
void dbscan::octreeRegionQuery(htr::OctreeGenerator *octreeGen, pcl::mod_pointXYZ &searchPoint,
      ...
      if (neighborPts.size() < minPts)
        ...
        for (int j = 0; j < neighborPts.size(); j++) {
          ...
          if (!visited[neighborPts[j]]) {
            ...
            octreeRegionQuery(octreeGen, octreeGen->getCloud()->points.at(neighborPts[j]), eps,
                              &neighborPts_);
            //
