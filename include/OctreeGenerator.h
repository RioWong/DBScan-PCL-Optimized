...
/// Solution specific to microsoft.
  ...
  void initRandomCloud(const float width, const float height, const float depth,
                       const int numOfPoints);
  void initCloudFromFile(string fileName);

  void readCloudFromFile(const char *filename);

  template <typename T> void initCloudFromVector(const vector<T> &points);
...
template <typename T> void OctreeGenerator::initCloudFromVector(const vector<T> &points) {
