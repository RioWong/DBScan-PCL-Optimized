...
void init(int argc, char **argv, bool show, std::string extension) {
  ...
  if (argc == 2) {
    ...
    pcl::console::print_info("\n- octree resolution: ");
    pcl::console::print_value("%d", groupA.size() * 0.001);
    pcl::console::print_info("\n- epsilon: ");
    pcl::console::print_value("%d", groupA.size() * 0.001);
    pcl::console::print_info("\n- min points: ");
    pcl::console::print_value("%d", 10);
    pcl::console::print_info("\n- max points: ");
    pcl::console::print_value("%d", 100);
    dbscan.init(groupA, groupA.size() * 0.001, groupA.size() * 0.001, 10, 100); /*RUN DBSCAN*/
  } else {
  ...
  if (extension == "txt") {
    ...
    for (auto &cluster : dbscan.getClusters()) {
      ...
      for (auto &point : cluster.clusterPoints) {
        ...
        fout << point.x << " " << point.y << " " << point.z << " " << r << " " << g << " " << b
             << std::endl;
      }
    ...
    for (auto &cluster : dbscan.getClusters()) {
      ...
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cluster_pcd(
    ...
    for (auto &cluster : dbscan.getClusters()) {
      ...
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cluster_ply(
          new pcl::PointCloud<pcl::PointXYZRGB>());
  ...
  if (show) {
    ...
    if (cloud->points[0].r <= 0 and cloud->points[0].g <= 0 and cloud->points[0].b <= 0) {
      pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> color_handler(cloud, 255,
                                                                                       255, 0);
